...
processar_midia() {
  ...
  # Verificar se as legendas já existem
  if [ "$tipo" = "Filmes" ]; then
    for diretorio in "${NOVOS_FILMES[@]}"; do
      legenda_existe=$(executar_no_container "find \"$diretorio\" -maxdepth 1 -type f -iname '*.srt'")
      if [ -z "$legenda_existe" ]; then
        echo "🔍 Buscando legenda para: $diretorio" | tee -a "$LOGFILE"
        docker exec filebot /opt/filebot/filebot -get-subtitles "$diretorio" -non-strict --lang pb --output srt --encoding utf-8 >> "$LOGFILE" 2>&1
        if [ $? -eq 0 ]; then
          echo "✅ Legenda baixada com sucesso para $diretorio" | tee -a "$LOGFILE"
        else
          echo "⚠️ Falha ao baixar legenda para $diretorio" | tee -a "$LOGFILE"
        fi
      else
        echo "🟡 Legenda já existe para $diretorio, ignorando..." | tee -a "$LOGFILE"
      fi
    done
  elif [ "$tipo" = "Séries" ]; then
...
if [ "$ARQUIVOS_PROCESSADOS" -gt 0 ]; then
  ...
  if [ $FILMES_STATUS -eq 0 ]; then
    ...
    chown -R mediauser:mediagroup "/mnt/media/data/media/movies"
  ...
  if [ $SERIES_STATUS -eq 0 ]; then
    ...
    # Definir o proprietário para todos os arquivos e diretórios
    chown -R mediauser:mediagroup "/mnt/media/data/media/tv"
